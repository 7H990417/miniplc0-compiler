...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
		...
		while (true) {
			...
			// 变量可能没有初始化，仍然需要一次预读
			if (isDeclared(inf.value().GetValueString()))
				return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrDuplicateDeclaration);
			...
			// '<表达式>'
			auto err = analyseConstantExpression();
			if (err.has_value())
				return err;
			// ';'
			next = nextToken();
			if (!next.has_value() || next.value().GetType() != TokenType::SEMICOLON)
				return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNoSemicolon);
			addVariable(inf.value());
			_instructions.emplace_back(Operation::STO, getIndex(inf.value().GetValueString()));
		}
		return {};
	}
	...
	std::optional<CompilationError> Analyser::analyseConstantExpression(int32_t& out) {
		...
		if (next.value().GetType() != TokenType::UNSIGNED_INTEGER &&
				next.value().GetType() != TokenType::PLUS_SIGN &&
		...
		switch (next.value().GetType()) {
			...
			case MINUS_SIGN:
				next=nextToken();
				int min;
				min=std::any_cast<int32_t>(next.value().GetValue());
				out=-min;
		}
		return {};
	}
	...
	std::optional<CompilationError> Analyser::analyseAssignmentStatement() {
		...
		_instructions.emplace_back(Operation::STO, getIndex(ident.value().GetValueString()));
		return {};
	}
